...
Cypress.Commands.add('criarConta15', () => {
    cy.fixture('criarConta').then((dados) => {
        ...
        cy.get('[data-qa="password"]').type(dados.Teste24.password);
        cy.get('#days').select('3');
        cy.get('#months').select('March');
        cy.get('#years').select('1996');
        cy.get('#newsletter').click();
        cy.get('#optin').click();
        cy.get('#first_name').type(dados.Teste24.first_name);
        cy.get('#last_name').type(dados.Teste24.last_name);
        cy.get('#company').type(dados.Teste24.company);
        cy.get('#address1').type(dados.Teste24.address1);
        cy.get('#address2').type(dados.Teste24.address2);
        cy.get('#country').scrollIntoView().select('Canada');
        cy.get('#state').type(dados.Teste24.state);
        cy.get('#city').type(dados.Teste24.city);
        cy.get('#zipcode').type(dados.Teste24.zipcode);
        cy.get('#mobile_number').type(dados.Teste24.mobile_number);
...
Cypress.Commands.add('verificarInfosEseguirParaPagamento', () => {
    cy.fixture('criarConta').then((dados) => {
        ...
        cy.get('#address_delivery').within(() => {
            ...
            cy.contains('li', `${dados.Teste24.first_name} ${dados.Teste24.last_name}`).should('be.visible');
            
            cy.contains('li', `${dados.Teste24.address1}`).should('be.visible');
            cy.contains('li', `${dados.Teste24.address2}`).should('be.visible');
            
            cy.contains('li', `${dados.Teste24.city} ${dados.Teste24.state} ${dados.Teste24.zipcode}`).should('be.visible');

            cy.contains('li', dados.Teste24.country).should('be.visible');
            cy.contains('li', dados.Teste24.phone).should('be.visible');
        });
